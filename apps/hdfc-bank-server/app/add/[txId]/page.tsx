...
async function validateToken(token: string, txId: string) {
    try {
      ...
      const realTokenRecord = await db.offRampTransaction.findUnique({
        where: {
          id: Number(txId),
        },
        select: {
      ...
      const decode:any =  jwt.verify(realTokenRecord.token,process.env.JWT_SECRET as string) 
      ...
      return { error: 'An unexpected error occurred. Please try again later.'+error.message };
    }
  }
  ...
  export default async function MyPage({
    ...
    if (validation.error) {
      ...
      if(validation.valid){
        return <CardForm2  amount ={amount} token={validation.token} txId={txId} tokenInfo={validation.tokenInfo}></CardForm2>
      }
