...
export const Appbar = () => {
  ...
  return (
    <>
      <motion.div className="h-16 md:p-7 p-3 w-full flex bg-[#f9f7f6] border-b-[1px] items-center md:w-[100%] z-20 fixed top-0 justify-between">
        <div
          onClick={() => {
            router.push('/');
          }}
        ...
        {session.status === 'unauthenticated' && (
          <div className="space-x-4 flex md:block">
            <motion.button
              onClick={() => {
                router.push('/signin');
              }}
            ...
            <motion.button
              onClick={() => {
                router.push(
                  'http://vault.merchant-app.vishawdeepsingh.in/api/auth/signin'
                );
              }}
              className=" bg-[#5741d7] text-white rounded-md bg-primary px-4 md:py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/80 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
              whileHover={{ scale: 1.1 }}
        ...
        {session.status === 'authenticated' && (
          ...
          >
            ...
            >
              ...
              <motion.li
                className="w-full p-2 md:hidden text-left hover:bg-slate-200"
                variants={itemVariants}
              >
                {session.status === 'authenticated' && (
                  <>
